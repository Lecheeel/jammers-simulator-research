```text
      _                              ____  _                 _       _
     | | __ _ _ __ ___   ___ _ __   / ___|| |__   ___  _ __ | |_ ___| |__   ___ _ __
  _  | |/ _` | '_ ` _ \\ / _ \\ '__|  \\___ \\| '_ \\ / _ \\ '_ \\| __/ __| '_ \\ / _ \\ '__|
 | |_| | (_| | | | | | |  __/ |      ___) | | | |  __/ | | | | || (__| | | |  __/ |
  \\___/ \\__,_|_| |_| |_|\\___|_|     |____/|_| |_|\\___|_| |_|\\__\\___|_| |_|\\___|_|
```
...
本项目对应 **2026 年全国大学生数学建模竞赛（CUMCM）B 题**，公开资料中常用的英文描述为 **Fast Automatic Localization and Neutralization of Radio Jammers**，中文可译为“无线电干扰源的快速自动定位与清除”。竞赛运行时，参赛程序通过官方 Windows 模拟器完成受限时间内的干扰源测量、定位和清除；模拟器对外暴露 HTTP REST 交互面，客户端需要遵守进入、测量、清除、退出等协议生命周期。

本仓库的定位是“逆向研究 + 本地自动化兼容层”：

- `sample/jammers-simulator.exe` 是固定哈希的本地样本，用于离线研究；
- `automation/` 是我们重建的仿真器和 loopback 协议服务，不是官方服务器；
- `tests/` 用于验证几何、虚拟时间、状态机、请求校验和幂等行为；
- 任何竞赛解题策略、模型参数和正式成绩都不由本仓库代替或保证。

更完整的背景、边界和合规说明见 [`docs/contest-info.md`](docs/contest-info.md)。具体赛题、运行时和纪律要求请以组委会正式通知和官方发布为准。
